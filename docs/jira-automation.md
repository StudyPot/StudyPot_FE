...
7. 필요하면 `branch_suffix`에 브랜치 뒤에 붙일 영문 slug를 입력합니다.
8. 실행이 끝나면 워크플로 요약에서 Jira 키, Jira 링크, 생성된 원격 브랜치를 확인합니다.

예시:

- `summary`: `회원가입 페이지 UI 작업`
- `description`: `이메일, 비밀번호, 확인 비밀번호 입력 폼을 구성한다.`
- `branch_suffix`: `signup-page`

성공하면 보통 이런 흐름으로 이어집니다.

1. Jira에서 `STUDY-123` 같은 이슈 키가 생성됩니다.
2. GitHub 원격 저장소에 `STUDY-123/signup-page` 같은 브랜치가 자동 생성됩니다.
3. PR 제목도 `[FEAT] STUDY-123 회원가입 페이지 추가`처럼 작성합니다.

로컬에서 이어서 작업할 때는 아래 명령으로 가져오면 됩니다.

```bash
git fetch origin
git switch --track origin/STUDY-123/signup-page
```
...
- 원격 브랜치까지 자동으로 만들어두면 팀원이 같은 이름 규칙으로 바로 작업을 시작할 수 있습니다.
